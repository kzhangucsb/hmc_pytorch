"""
Created on Sun Dec  8 14:31:15 2019
...
from gloss import poisson_exp_loss
...
with open('../data/{}.pickle'.format(nl), 'rb') as f:
    p = pickle.load(f)
...
model = cp(size, rank, alpha=2, beta=0.2, c = 2, d=1e4)#1, beta = 1, c = 1)#5, 1e4) # beta=0.2, c=10)
...
plt.legend()
plt.show()
with open('../models/cp_poisson_model_{}_{}.pth'.format(nl, int(np.round(time.time()))), 'wb') as f:

...
class hmcsampler(Optimizer):
    ...
    def __init__(self, params, num_steps_in_leap = 32, max_step_size=0.05,
                 sampler=None, sampling_interval = None, **kwargs):
        defaults = {'B': 0,
                  'frac_adj': 0,
                  'max_length': 1,
                  'mass': 1,
...
#            os.makedirs(self.samples_dir)
    ...
    def step(self, closure=None):
        ...
        if self.sampling_interval is None:
            s = (self.step_eclipsed > 0) and (self.step_eclipsed % self.num_steps_in_leap == 0)
        else:
            s = self.time_eclipsed > self.sampling_interval
        if s:
            # keep a sample
    ...
    def _sample(self):
        ...
        self.sampler.save()
        self.weights.append(self.time_eclipsed)
#        s = []
#        for group in self.param_groups:
#            s.append([p.clone().detach().cpu() for p in group['params']])

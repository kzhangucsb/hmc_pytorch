...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  6 17:27:37 2019
...
if __name__ == '__main__':
    ...
    with open('ktensor_noise_1em2.pickle', 'rb') as f:
        p = pickle.load(f)
...
#    model.load_state_dict(state_dict)
    sampler = hmcsampler(model, train_loader, criterion, dataloader_test=test_loader,#g=g, phi=phi,
                         m = 1e-4, frac_adj=0, mass = {'lamb':1e4},
                         max_step_size = 1e-3, max_length=0.1, max_length_t=0.01, T = 1e-6, 
                         lb = 0.01, num_steps_in_leap=32, regularizer=cp.prior_theta)
#    samples_discard, status = sampler.sample(500)
##    sampler.m = 0.01
    with open('cp_bayes_model.pth', 'rb') as f:
